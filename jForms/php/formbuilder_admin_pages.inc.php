 	...
 	function formbuilder_options_editFormObject($form_id)
 	{
		...
		if($fe->isReadyToValidate())
		{
			if($fe->validate())
			{
				if($form->saveToDB())
				{
					$message = sprintf(__("Your form has been saved.", 'formbuilder'), "<a href='" . FB_ADMIN_PLUGIN_PATH . "'>", "</a>");
...
#					$response_data = $tableResponses->load_row($response_id);
 	...
 	function formbuilder_get_required_types()
 	{
 		$all_required_types = array(
			...
			'Confirm email address'=>__("Requires an email address", 'formbuilder'),
			'Phone number'=>__("Requires a phone number.", 'formbuilder'),
			'Any number'=>__("Requires some sort of number.", 'formbuilder'),
			'Web address'=>__("Requires a valid URL.", 'formbuilder'),
			'Single word'=>__("Requires a single word.", 'formbuilder'),
			'Date (dd/mm/yyyy)'=>__("Requires a date stamp in the form of (dd/mm/yyyy)", 'formbuilder'),
		);
 	...
 	function formbuilder_get_field_types()
 	{
 		$all_field_types = array(
			...
			'Date'=>__("Date selection field.", 'formbuilder'),
			'Radio buttons'=>__("Radio selection buttons.  Enter one per line in the field value.", 'formbuilder'),
			'Dropdown'=>__("Dropdown box.  Enter one value per line in the field value.", 'formbuilder'),
			...
			'Recipient select'=>__("A special selection dropdown allowing the visitor to specify an alternate form recipient.  Enter values in the form of email@domain.com|Destination Name.", 'formbuilder'),
		...
		if(function_exists('imagecreate')) 
			$all_field_types['CAPTCHA'] = __("Special field on the form for displaying CAPTCHAs.  Field name is used for identifying the field.  Field label is used to give the visitor further instruction on what to fill out.", 'formbuilder');
