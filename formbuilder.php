...
/*
Plugin Name: jForms
Plugin URI: http://www.jamestarpey.com
Description: Build your own forms
Author: James Tarpey
Version: 0.1
Author URI: http://www.jamestarpey.com

*/
	
	define("FORMBUILDER_VERSION_NUM", "0.1");
	...
	function formbuilder_admin_menu()
	{
		...
		add_management_page(__('jPP', 'formbuilder'), __('jForms', 'formbuilder'), 'publish_posts', FORMBUILDER_FILENAME, 'formbuilder_options_page');
	...
	function formbuilder_plugin_notice( $plugin ) {
		...
		if($version != FORMBUILDER_VERSION_NUM)
		{
		 	if( $plugin == FORMBUILDER_PLUGIN_KEY && function_exists( "admin_url" ) )
				echo '<td colspan="5" class="formbuilder-plugin-update">jForms must be configured. Go to <a href="' . admin_url( 'tools.php?page=formbuilder.php' ) . '">the admin page</a> to enable and configure the plugin.</td>';
		}
	}
	...
	function formbuilder_css()
	{
		...
		{
			$css_path = FORMBUILDER_PLUGIN_URL . "css/style.css";
			?>
